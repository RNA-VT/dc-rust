...
pub fn server(
    ...
    // http://<sta ip>/ handler
    server.fn_handler::<EspIOError, _>("/", Method::Get, |request| {
        Ok({
            let html = index_html();
    ...
    server.fn_handler::<EspIOError, _>("/specification", Method::Post, move |request| {
        let json_response =
            serde_json::to_string(&specification).expect("Failed to Serialize JSON");

        let mut response = request
            .into_ok_response()
            .expect("Failed to create response from request");
        response
            .write(json_response.as_bytes())
            .expect("Failed to write JSON Response");
        Ok(())
    })?;

    server.fn_handler::<EspIOError, _>("/health", Method::Post, |_request| Ok(()))?;
